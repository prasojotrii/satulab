			...
			<div class="card-body">
				...
				<div class="tab-content p-2 text-muted">
						...
						<table class="table" style="border: none; width: 100%;">
							<tbody>
								<tr>
									...
									<td style="border: none; padding: 0px;" width="47%">: <span id="info_material"></span></td>
								</tr>
								<tr>
					...
					<div class="tab-pane" id="lab" role="tabpanel">
						...
						<div style="display: none;" id="input_analis_lab">
							...
							<table class="table" style="border: none; width: 100%;">
								<tbody>
									...
									<tr>
										...
										<td style="border: none; padding: 0px;" width="65%">: <select class="form-select form-select-sm select2" id="nama_analis" name="nama_analis">
												<option value="" selected disabled>Pilih Nama Analis</option>
											</select></td>
										<th style="border: none; padding: 0px;" width="15%">
										<td style="border: none; padding: 0px;" width="5%"> </td>
							...
							<div class="form-group row mt-3" style="display: none;">
								<label for="fileUpload" style="color: black;" class="col-form-label col-2">Unggah File:</label>
								<div class="col-10">
						...
						<div id="form_hasil_lab">
							<table class="table" style="border: none;">
								<tbody>
									<tr>
										<th style="border: none; padding: 0px;" width="15%">Nama Analis </th>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#btn_update_analis').on('click', function() {
			...
			if (nama_analis) {
				...
				var selectedAnalis = $('#nama_analis').val();
				// alert(selectedAnalis);
				// alert(id_req);
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					$.ajax({
						...
						type: 'POST',
						data: {
							id_req: id_req, // Pastikan variabel id sudah dideklarasikan sebelumnya
	...
	function loadLabParameters() {
		...
		get_info_nama_analis(id_req);

		// setupButtonHandlers(id_req);
		...
		$('#uploadButton').on('click', function() {
			...
			form_data.append('id_req', id_req);
				...
				success: function(response) {
					try {
						...
						if (jsonResponse.status) {
							...
							$('#viewFileButton').data('file-id', id_req);
							$('#deleteFileButton').data('file-id', id_req);
						} else {
		...
		// $('#nama_analis').change(function() {

		// });
	}

	function get_info_nama_analis(id_req) {
	...
	function getAnalisLab(id_req) {
		...
		$.ajax({
			url: "<?php echo site_url('analisa/get_analis_lab2'); ?>",
			type: "POST",
			data: {
				id_req: id_req
			},
	...
	function initSelect2(defaultAnalisLab) {
		...
		}).val(defaultAnalisLab);
	}

	function fetchAnalisList(defaultAnalisLab, id_req) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data.status) {
					...
					namaAnalisSelect.val(defaultAnalisLab);
	...
	function updateAnalisLabOnSelect() {
	...
	function getAnalisaLab(id_req, progress) {
						...
						success: function(response) {
                            ...
                            `);
                                ...
                                <div class="tab-pane ${i === 1 ? 'active' : ''}" id="sample${i}" role="tabpanel">
                                    <table id="parameter-table-lab-${i}" class="table table-bordered" style="border: 1px solid #dee2e6; padding: 2px 5px; width: 100%;">
                                        <thead>
                                            <tr>
                                                <th style="padding: 2px 5px;">No</th>
                                                <th style="padding: 2px 5px;">Parameter</th>
        ...
        `;
										...
										// Fungsi untuk mengonversi notasi 'x*10-n' atau 'x*10+n' menjadi hasil perhitungan
										function convertToScientific(input) {
											return input.replace(/(\d+(?:\.\d+)?)\*10([+-]?\d+)/g, function(match, number, exponent) {
												// Konversi ke bentuk angka desimal
												let result = parseFloat(number) * Math.pow(10, parseInt(exponent));

												// Batasi hingga maksimal 10 desimal dan hapus trailing zero yang tidak diperlukan
												return parseFloat(result.toFixed(10)).toString();
											});
										}

										// Event untuk menampilkan konversi di input field tanpa memicu looping
										$('.result-input, .valid-input').on('focusout', function() {
											var originalText = $(this).val();
											var convertedText = convertToScientific(originalText);

											// Tampilkan hasil konversi di input field
											$(this).val(convertedText);
										});

										// Event untuk mentrigger fungsi saat focusout atau Enter, dan fokus ke baris berikutnya
										$('.result-input, .valid-input').on('focusout keypress', function(e) {
											if (e.type === 'focusout' || e.which === 13) { // Fokus keluar atau tekan Enter
												var originalText = $(this).val();
												var convertedText = convertToScientific(originalText);
												$(this).val(convertedText);

												// Pindah ke input field di baris berikutnya
												var $nextRowInput = $(this).closest('tr').next('tr').find(`.${$(this).attr('class')}`);
												...
												// Ambil nilai yang sudah dikonversi
												var result = $(this).closest('tr').find('.result-input').val();
												var valid = $(this).closest('tr').find('.valid-input').val();
										...
										// Event untuk mengirim data setelah perubahan (saat change di-trigger)
				...
				} else if (progress === "Analisa Lab selesai" || progress === "Approval Manager QC" || progress === "Karantina Selesai") {
					$.ajax({
						url: 'sample/get_jumlah_sample', // Ganti dengan URL endpoint Anda
						type: 'POST',
						data: {
	...
	function salinHasilKeSemua(id_req, mstrchar) {
		var clickedElement = $(event.target);
		var idSpec = clickedElement.closest('tr').find('.result-input').data('id-spec');
		var resultValue = clickedElement.closest('tr').find('.result-input').val();
		var validValue = clickedElement.closest('tr').find('.valid-input').val();

		if (!idSpec || !resultValue) {
			alert('Data tidak lengkap untuk disalin.');
			return;
		}

		$.ajax({
			url: "<?php echo site_url('analisa/copy_to_all'); ?>",
			type: "POST",
			dataType: 'json', // Ini akan memastikan respons diuraikan sebagai JSON
			data: {
				id_req: id_req,
				mstrchar: mstrchar,
				id_spec: idSpec,
			...
			success: function(response) {
				console.log(response); // Memeriksa respons JSON
				if (response.status === 'success') {
					Swal.fire({
						icon: 'success',
						title: 'Berhasil',
						text: 'Data berhasil direplikasi ke semua baris yang sesuai.'
					});
				} else {
					Swal.fire({
						icon: 'error',
