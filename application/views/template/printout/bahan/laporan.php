<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta content="Laporan Stok Bahan Laboratorium Sido Muncul" name="description" />
    <meta content="" name="author" />
    <!-- App favicon -->
    <title><?= $title; ?></title>
    <link rel="shortcut icon" href="<?= base_url(); ?>assets/images/favicon.ico">
    <style type="text/css">
        p {
            margin: 5px 0 0 0;
        }

        p.footer {
            text-align: right;
            font-size: 11px;
            border-top: 1px solid #D0D0D0;
            line-height: 32px;
            padding: 0 10px 0 10px;
            margin: 20px 0 0 0;
            display: block;
        }

        .bold {
            font-weight: bold;
        }


        #footer {
            clear: both;
            position: relative;
            height: 40px;
            margin-top: -40px;
        }
    </style>
</head>

<body style="font-size: 12px">

    <table width="100%">
        <tr>
            <td width="20%" style="text-align: center;"><img src="data:image/png;base64," style="width: 95px;"></td>

            <td width="60%" style="text-align: center; "> <span style="font-size: 14px;font-family: Verdana, Geneva, Tahoma, sans-serif">
                    <b>LABORATORIUM ANALISA PT.SIDO MUNCUL</b></span><br>
                <span style="font-size: 8px;font-family: Verdana, Geneva, Tahoma, sans-serif">Laboratorium Penguji</span><br>
                <span style="font-size: 8px;font-family: Verdana, Geneva, Tahoma, sans-serif">Jl.Soekarno Hatta Km. 28 Kec. Bergas-Klepu, Semarang Indonesia</span><br>
                <span style="font-size: 8px;font-family: Verdana, Geneva, Tahoma, sans-serif">Telp. (62-298)523515 (Hunting); Fax. (62-298)523509; E-mail: factory@sidomuncul.id, laboratanalisa.sm@gmail.com</span>
            </td>
            <td width="20%" style="text-align: center;">
                <div><span style="font-size: 8px;display: inline-block;border: 1px solid black; padding: 5px;">FR/PR/PM.6-5/2</span></div>
            </td>
        </tr>


    </table>

    <hr>
    <table width="100%">
        <tr style="text-align: center;">
            <td style="text-align: center;">
                <span style="font-size: 16px"><b> LAPORAN STOK REAGEN CAIR BAHAN KIMIA LABORATORIUM ANALISA <br>
                        <!-- PT. JAMU DAN FARMASI SIDOMUNCUL,Tbk<br>
                        Jl.Soekarno Hatta KM. 28 Kec.Bergas-Klepu, Semarang Indonesia 50552<br> -->

                    </b></span>
            </td>
        </tr>
    </table>
    <br>
    <br>

    <table style="border: 1px solid black;border-collapse: collapse;font-size: 11px" width="100%">
        <thead>
            <tr style="margin: 5px">
                <th colspan="10" style="font-size: 14px;border: 1px solid black;">REKAP STOK AKHIR</th>

            </tr>
            <tr style="margin: 5px">
                <?php
                // Ubah format tanggal dari '2023-8' menjadi 'Agustus 2023'
                $periode_bulan = date("F Y", strtotime($periode_bulan));
                ?>
                <th colspan="10" style="font-size: 14px;border: 1px solid black;"><?= $periode_bulan ?></th>

            </tr>
            <tr style="margin: 5px">
                <th style="border: 1px solid black;">No</th>
                <th style="border: 1px solid black;">Nama Bahan Kimia</th>
                <th style="border: 1px solid black;">Kemasan</th>
                <th style="border: 1px solid black;">Jumlah Awal</th>

                <th style="border: 1px solid black;">Masuk</th>
                <th style="border: 1px solid black;">Keluar</th>
                <th style="border: 1px solid black;">Sisa Akhir</th>

                <th style="border: 1px solid black;">Tanggal ED</th>
                <th style="border: 1px solid black;">Nomor Batch</th>
                <th style="border: 1px solid black;">Merk</th>

            </tr>
        </thead>
        <tbody>
            <!-- 
            <?php $filter_bahan ?> -->
            <?php $periode_bulan ?>

            <?php

            function formatData($data)
            {
                $counted_data = array_count_values($data);
                $formatted_data = [];
                foreach ($counted_data as $value => $count) {
                    $formatted_data[] = "$value ($count)";
                }
                return implode('<br>', $formatted_data);
            }

            if (!empty($list_stok)) {
                $grouped_data = [];
                $jumlah_baris_stok_awal = [];
                $jumlah_baris_stok_masuk = [];
                $jumlah_baris_stok_keluar = [];

                foreach ($list_stok as $item) {
                    $id_bahan = $item['id_bahan'];
                    $nama_bahan = $item['nama_bahan'];
                    $satuan = $item['satuan'];

                    if (!isset($grouped_data[$nama_bahan])) {
                        $grouped_data[$nama_bahan] = [
                            'id_bahan' => [],
                            'kemasan' => [],
                            'tanggal_ed' => [],
                            'no_batch' => [],
                            'merek' => [],
                            'jumlah_data' => 0,
                        ];
                    }

                    $grouped_data[$nama_bahan]['kemasan'][] = $item['kemasan'] . ' ' . $satuan;
                    $grouped_data[$nama_bahan]['tanggal_ed'][] = $item['tanggal_ed'];
                    $grouped_data[$nama_bahan]['no_batch'][] = $item['nomor_batch'];
                    $grouped_data[$nama_bahan]['merek'][] = $item['merek'];
                    $grouped_data[$nama_bahan]['id_bahan'][] = $item['id_bahan'];
                    $grouped_data[$nama_bahan]['jumlah_data']++;

                    // Inisialisasi jumlah baris stok_awal, stok_masuk, dan stok_keluar untuk setiap bahan
                    $jumlah_baris_stok_awal[$nama_bahan] = 0;
                    $jumlah_baris_stok_masuk[$nama_bahan] = 0;
                    $jumlah_baris_stok_keluar[$nama_bahan] = 0;

                    // Hitung jumlah baris stok_awal yang id_bahan = $item['id_bahan']
                    foreach ($stok_awal as $row) {
                        if ($row['id_bahan'] == $item['id_bahan']) {
                            $jumlah_baris_stok_awal[$nama_bahan]++;
                        }
                    }

                    // Hitung jumlah baris stok_masuk yang id_bahan = $item['id_bahan']
                    foreach ($stok_masuk as $row) {
                        if ($row['id_bahan'] == $item['id_bahan']) {
                            $jumlah_baris_stok_masuk[$nama_bahan]++;
                        }
                    }

                    // Hitung jumlah baris stok_keluar yang id_bahan = $item['id_bahan']
                    foreach ($stok_keluar as $row) {
                        if ($row['id_bahan'] == $item['id_bahan']) {
                            $jumlah_baris_stok_keluar[$nama_bahan]++;
                        }
                    }
                }

                $no = 1; // Variabel untuk nomor urut

                foreach ($grouped_data as $nama_bahan => $group) {
                    echo '<tr style="text-align: center;">';
                    echo '<td style="border: 1px solid black;">' . $no . '</td>';
                    echo '<td style="border: 1px solid black;">' . $nama_bahan . '</td>';
                    echo '<td style="border: 1px solid black;">' . formatData($group['kemasan']) . '</td>';
                    echo '<td style="border: 1px solid black;">' . $jumlah_baris_stok_awal[$nama_bahan] . '</td>';
                    echo '<td style="border: 1px solid black;">' . $jumlah_baris_stok_masuk[$nama_bahan] . '</td>';
                    echo '<td style="border: 1px solid black;">' . $jumlah_baris_stok_keluar[$nama_bahan] . '</td>';
                    echo '<td style="border: 1px solid black;">' . ($jumlah_baris_stok_awal[$nama_bahan] + $jumlah_baris_stok_masuk[$nama_bahan] - $jumlah_baris_stok_keluar[$nama_bahan]) . '</td>';
                    echo '<td style="border: 1px solid black;">' . formatData($group['tanggal_ed']) . '</td>';
                    echo '<td style="border: 1px solid black;">' . formatData($group['no_batch']) . '</td>';
                    echo '<td style="border: 1px solid black;">' . formatData($group['merek']) . '</td>';
                    echo '</tr>';
                    $no++;
                }
            } else {
                echo '<tr><td colspan="10" style="border: 1px solid black;">Tidak ada data stok yang tersedia.</td></tr>';
            }
            ?>








        </tbody>


    </table>


    </p>



    <br>




    <table width="100%">
        <tr>
            <td align="center" width="33,3%"></td>

            <td align="center" width="33,3%"></b></td>
            <td align="center" width="33,3%">Ungaran, <?php echo  date("d M Y") ?></td>
        </tr>
        <tr>
            <td align="center" width="33,3%">Petugas<br><br><br><br><br><u>( Oey, Stefanus.A. )</u></td>
            <td align="center" width="33,3%"></td>
            <td align="center" width="33,3%">Diperiksa Oleh<br><br><br><br><br>( ........................... )</u></td>
        </tr>
    </table>
    <table width="100%">

        <tr>
            <td align="center" width="33,3%"></td>
            <td align="center" width="33,3%">Disetujui Oleh<br><br><br><br><br><u>( Erni Rusmalawati )</u></td>
            <td align="center" width="33,3%"></td>
        </tr>
    </table>


    <!-- <p class="footer">
        <small>Halaman <b>1</b> dari <b>1</b></small>
    </p> -->


</body>

</html>