<?php
class Master_unit_sub_model extends CI_Model
{
    var $table = 'tb_master_unit_sub';
    var $column_order = [null, null, null, 'sub_unit_name', 'sub_unit_inisial', 'email_sub_unit'];
    //set column field database for datatable orderable

    var $column_search = ['sub_unit_name'];
    //set column field database for datatable searchable just firstname , lastname , address are searchable
    var $order = ['sub_unit_name' => 'asc']; // default order
    ...
    {
        $this->db->from($this->table);
        $this->db->where('id_unit_sub', $id_unit_sub);
        $query = $this->db->get();

        return $query->row();
    }

    public function get_data_by_id_unit($id_unit)
    {
        $this->db->from($this->table);
        $this->db->where('id_unit', $id_unit);
        $this->db->select('id_unit_sub, sub_unit_name');
        $query = $this->db->get();
    ...
    public function delete($id_unit_sub)
    {
        $this->db->where('id_unit_sub', $id_unit_sub);
