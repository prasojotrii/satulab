...
class Admin extends CI_Controller
{
	...
	public function Kirim_Email_Pemberitahuan()
	{
		...
		$no_invoice         = $this->input->post('id_order');
	...
	public function data_request_order()
	{
		...
		$data['title'] = 'Daftar Order Internal Laboratorium';
		// $this->load->view('admin/header', $data);
		$this->load->view('admin/list_request_order', $data);
	}
	...
	public function kategori_instrumen()
	{
		...
		$data['dataSession'] = $this->User_model->getDataUsername();
		$this->load->view('admin/header', $data);
		$this->load->view('admin/list_kategori_instrumen', $data);
	}
	...
	public function kategori_glassware()
	{
		...
		$data['dataSession'] = $this->User_model->getDataUsername();
		$this->load->view('admin/header', $data);
		$this->load->view('admin/list_kategori_glassware', $data);
	}
	...
	public function instrumen()
	{
		...
		// $this->load->view('admin/header', $data);
		$this->load->view('admin/header', $data);
		$this->load->view('admin/list_instrumen', $data);
	}

	public function glassware()
	{
		$data['dataKategori'] = $this->Glassware_model->dataKategori();
		$data['id_input'] = $this->Kalibrasi_Glassware_model->Membuat_Kode_Otomatis();
		...
		// $this->load->view('admin/header', $data);
		$this->load->view('admin/header', $data);
		$this->load->view('admin/list_glassware', $data);
	}
	...
	public function list_user()
	{
		...
		foreach ($list as $User_model) {
			...
			$row[] = '<a class="btn btn-primary btn-sm"  href="javascript:void(0)" >' . $User_model->username . '</a>';
	...
	public function list_order()
	{
		...
		foreach ($list as $Instrumen_Baru_model) {
			...
			$row[] = $Instrumen_Baru_model->status;
	...
	public function list_order_persetujuan()
	{
		...
		foreach ($list as $Order_Persetujuan_model) {
			...
			$row[] = $no;
			$row[] = '<a class="btn btn-primary btn-sm" onclick="Btn_Update(' . "'" . $Order_Persetujuan_model->id_order . "'" . ')" href="javascript:void(0)" >' . $Order_Persetujuan_model->id_order . '</a>';
			$row[] = '<a class="btn btn-primary btn-sm" onclick="Btn_Persetujuan(' . "'" . $Order_Persetujuan_model->id_batch . "'" . ')" href="javascript:void(0)" >' . $Order_Persetujuan_model->id_batch . '</a>';
			...
			$row[] = $Order_Persetujuan_model->status;
	...
	// <a class= "btn btn-outline-primary btn-sm" href="javascript:void(0)" title="Delete" onclick="Btn_Batal(' . "'" . $Instrumen_Baru_model->id_order . "'" . ')"><i class="fas fa-times"></i> </a>
	public function list_order_baru()
	{
		$list = $this->Instrumen_Baru_model->get_datatables();
	...
	public function list_kategori_glassware()
	{
		...
		foreach ($list as $Kategori_Glassware_model) {
			...
			$row[] = '<a class="btn btn-primary btn-sm" style="width:120px;height:28px" href="/labsm/admin/glassware/' . $Kategori_Glassware_model->id_instrumen . '" >' . $Kategori_Glassware_model->id_instrumen . '</a>';
	...
	public function list_kategori_instrumen()
	{
		...
		foreach ($list as $Kategori_Instrumen_model) {
			...
			$row[] = $no;
			$row[] = '<a class="btn btn-primary btn-sm" style="width:120px;height:28px" href="/labsm/admin/instrumen/' . $Kategori_Instrumen_model->id_instrumen . '" >' . $Kategori_Instrumen_model->id_instrumen . '</a>';
	...
	public function laporan_kalibrasi()
	{
		...
		$data['dataSession'] = $this->User_model->getDataUsername();
		$this->pdf->load_view('admin/printout/kalibrasi/laporan', $data);
	}
	...
	public function lembar_kalibrasi()
	{
		...
		$this->pdf->load_view('admin/printout/kalibrasi/lembarkerja', $data);
	}
	...
	public function Tambah_order_persetujuan()
	{
		// $this->_validate();
		...
		$data = array(
			...
			'tanggal_buat' => $this->input->post('info_tanggal_waktu'),
		);
		...
		if ($this->email->send()) {
			$insert = $this->Order_Persetujuan_model->save($data);
			echo json_encode(array("status" => TRUE));
		} else {
	...
	public function Edit_oder_persetujuan($id_batch)
	{
		$data = $this->Order_Persetujuan_model->get_by_batch($id_batch);
	...
	public function Update_Order_Baru()
	{
		...
		$this->Instrumen_Baru_model->update(array('nomor' => $this->input->post('nomor')), $data);
		echo json_encode(array("status" => TRUE));
	}
	...
	public function Update_Order_Status3()
	{
		...
		$data = array(
			...
			'status' => $this->input->post('info_status_order'),
			'last_edit' => $this->input->post('info_last_edit'),
		);
		$this->Order_model->update(array('id_order' => $this->input->post('info_id_order')), $data);
		echo json_encode(array("status" => TRUE));
	...
	public function Update_disetujui_Kaunit()
	{
		...
		$this->Order_Persetujuan_model->update(array('id_batch' => $this->input->post('id_batch')), $data3);
	...
	public function Hapus_order_persetujuan($id_batch)
	{
		$this->Order_Persetujuan_model->delete_by_batch($id_batch);
		echo json_encode(array("status" => TRUE));
	}
	...
	public function list_aset_glassware()
	{
		...
		foreach ($list as $Glassware_model) {
			...
			<a class= "btn btn-outline-primary btn-sm" href="javascript:void(0)" title="Kalibrasi Instrumen" onclick="Btn_Kalibrasi(' . "'" . $Glassware_model->id_aset . "'" . ')">Kalibrasi</i> </a>
	...
	public function list_riwayat_instrumen()
	{
		...
		foreach ($list as $Riwayat_Instrumen_model) {
			...
			if ($Riwayat_Instrumen_model->id_laporan == 0) {
				...
				$cetak = '<a class= "btn btn-outline-primary btn-sm" target="_blank" href="http://193.13.7.3/labsm/admin/lembar_kalibrasi/' . $Riwayat_Instrumen_model->id_laporan  . '" title="Lembar kerja Kalibrasi"><i class="fa-regular fa-file-powerpoint"></i> </a>
		
				<a class= "btn btn-outline-primary btn-sm" target="_blank" href="http://193.13.7.3/labsm/admin/laporan_kalibrasi/' . $Riwayat_Instrumen_model->id_laporan  . '" title="Laporan Kalibrasi "><i class="fas fa-print"></i> </a>';
			}
